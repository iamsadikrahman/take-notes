...
function editNoteFunc(index) {
  notes[index].editNote = true;
}

function focusout(index) {
  notes[index].editNote = false;
}
</script>
...
<template>
  <section>
    <header class="flex justify-between p-4 bg-gray-200 rounded mb-4">
      ...
      <button
        @click="showModal"
        class="bg-blue-500 text-white px-4 py-2 rounded text-2xl"
        type="button"
      >
        Take Notes+
      </button>
    ...
    <main>
      <Modal :size="size" v-if="isShowModal" @close="closeModal">
        <template #header>
          <div class="flex items-center text-lg">Write Your Note...</div>
        </template>
        <template #body>
      ...
      <section class="flex space-x-4 flex-wrap">
        <div
          ...
          :key="index"
          class="border p-5 rounded-lg h-[400px] w-[400px] flex flex-col justify-between overflow-auto"
        ...
        >
          ...
          <button
            @click="editNoteFunc(index)"
            type="button"
            class="border p-2 bg-black text-white"
          >
            Edit
          </button>
          <button
